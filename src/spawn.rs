...
//! `spawn` — launch another application from a dynamic application (C28, §17.6).
//!
...
//! launch path crosses the SWI boundary (§17.6). The kernel performs the real
...
/// arrays (§17.6). The kernel enforces its own (stricter) limits during copy-in;
/// these stop a runaway `argv`/`envp` in the shared address space from scanning
/// without bound (design §9.2).
...
fn string_view(s: *const c_char) -> Option<BlueOsStringView> {
    ...
    // SAFETY: `s` is a caller-supplied NUL-terminated string. Phase 1 runs in a
    // shared privileged address space without a fault-safe page table, so a
    // malformed pointer is the caller's own memory-safety bug (design §9.2); the
...
/// Stable C ABI `spawn(path, argv, envp)` (C28, §17.6).
///
/// Returns the `ApplicationLaunch` syscall result: a generation handle on
/// success, or a negative errno. Phase 1's kernel launch glue still returns
/// `ENOSYS` until the C26/C28 copy-in lands; the handle encoding is defined by
/// that kernel handler, never by librs (§17.6).
#[linkage = "weak"]
#[no_mangle]
pub extern "C" fn spawn(
    ...
    let rc = bk_syscall!(ApplicationLaunch, &request as *const BlueOsApplicationLaunchRequest);
    rc as c_long
}
